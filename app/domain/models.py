...
class ProposedRemediation(BaseModel):
    """A PR-ready proposal generated by the incident distiller.

    Each instance represents a synthesised fix for one or more related incidents.
    The `markdown_diff` field is a unified-diff string that passes
    ``git apply --check`` — it is never auto-applied; always human-reviewed.
    """
    ...
    source_incident_ids: list[str]  # incidents that motivated the proposal
    created_at: datetime = Field(default_factory=datetime.utcnow)
